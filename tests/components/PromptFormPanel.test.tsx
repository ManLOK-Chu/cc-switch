...
vi.mock("@/lib/api", () => ({
  promptsApi: {
    getPromptCommonSnippet: vi.fn().mockResolvedValue(""),
    setPromptCommonSnippet: vi.fn().mockResolvedValue(undefined),
  },
}));
...
describe("PromptFormPanel", () => {
  ...
  it("stays open when the parent write lock rejects a save", async () => {
    ...
    const onClose = vi.fn();
    renderWithQueryClient(
      <PromptFormPanel appId="codex" onSave={onSave} onClose={onClose} />
    );
