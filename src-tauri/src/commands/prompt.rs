...
#[tauri::command]
pub async fn get_prompt_common_snippet(
    app: String,
    state: State<'_, AppState>,
) -> Result<Option<String>, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    PromptService::get_prompt_common_snippet(&state, app_type).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn set_prompt_common_snippet(
    app: String,
    value: String,
    state: State<'_, AppState>,
) -> Result<(), String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    PromptService::set_prompt_common_snippet(&state, app_type, value).map_err(|e| e.to_string())
}
