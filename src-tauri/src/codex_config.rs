...
"#;
    ...
    fn build_simplified_catalog_preserves_native_profile_overrides() {
        ...
        // MiniMax-M3 的 text+image 与当前能力推断一致，因此会折叠；
        // 推断结果不会成为粘滞的隐藏覆盖，后续能力注册表修正仍可生效。
        assert!(
            minimax.get("inputModalities").is_none(),
            "inferred input_modalities should not become a sticky hidden override"
        );
        assert_eq!(
