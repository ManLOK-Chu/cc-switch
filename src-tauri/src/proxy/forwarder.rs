...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        let preliminary_force_identity_encoding = needs_transform
            || codex_responses_to_chat
            || codex_responses_to_anthropic
            || preliminary_request_is_streaming;
        ...
        if thinking_summary.is_empty() {
            log::info!("[{tag}/{source}] >>> 请求目标: {target_for_log} (model={request_model})");
        } else {
            let thinking_summary = thinking_summary.join(", ");
            log::info!(
                "[{tag}/{source}] >>> 请求目标: {target_for_log} (model={request_model}, {thinking_summary})"
            );
        }
        log::debug!(
            "[{tag}/{source}] >>> 请求体已准备: bytes={}, hash={} (content omitted)",
            body_bytes.len(),
...
fn should_force_identity_encoding(
    ...
    should_force_identity_encoding_for_request(false, false, false, endpoint, body, headers)
}
...
/// 已知等级按原值记录；未知字符串只记录哈希，避免异常请求把任意正文写入日志。
fn extract_thinking_summary(body: &Value) -> Vec<String> {
        ...
        .and_then(|v| v.as_str())
    {
        if t == "enabled" {
            ...
            out.push(format!("thinking={}", summarize_thinking_value_for_log(t)));
        }
    }
    ...
    if let Some(effort) = body.get("reasoning_effort").and_then(|v| v.as_str()) {
        out.push(format!(
            "reasoning_effort={}",
            summarize_thinking_value_for_log(effort)
        ));
    }
...
fn summarize_thinking_value_for_log(value: &str) -> String {
    const KNOWN_VALUES: &[&str] = &[
        "adaptive", "enabled", "disabled", "none", "minimal", "low", "medium", "high", "xhigh",
        "max",
    ];
    ...
    format!("other[{}]", short_value_hash(Some(&value)))
}
...
mod tests {
    ...
    #[test]
    fn extract_thinking_summary_hashes_unknown_values() {
        let body = serde_json::json!({
            "thinking": {"type": "unexpected-secret-value"}
        });
        let summary = super::extract_thinking_summary(&body);

        assert_eq!(summary.len(), 1);
        assert!(summary[0].starts_with("thinking=other["));
        assert!(!summary[0].contains("unexpected-secret-value"));
    }
}
